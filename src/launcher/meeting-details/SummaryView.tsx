...
export function SummaryView({ meeting, generation, onExport }: SummaryViewProps) {
  ...
  const handleCopy = useCallback(async () => {
    ...
    if (text) {
      try {
        ...
        showToast(t("launcher.details.summary.copied"), "success");
      } catch { showToast(t("launcher.details.summary.copyFailed"), "error"); }
    }
      ...
      <div className="p-5">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm text-primary">
            ...
            <span className="font-medium">{t("launcher.details.summary.generating")}</span>
          </div>
          <button
          ...
          >
            ...
            {t("launcher.details.summary.cancel")}
          </button>
        </div>
        <div>
  ...
  if (generation.error) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-16">
        ...
        >
          {t("launcher.details.summary.regenerate")}
        </button>
      </div>
    );
  }
      ...
      <div className="flex h-full flex-col">
        <div className="flex items-center gap-2 px-5 py-2.5 border-b border-border/20">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground/50">
            ...
            <span>{t("launcher.details.summary.aiGenerated")}</span>
          </div>
          ...
          >
            ...
            {t("launcher.details.summary.copy")}
          </button>
          <button
            onClick={onExport}
            className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-muted-foreground/60 hover:bg-secondary hover:text-foreground transition-colors cursor-pointer"
          >
            ...
            {t("launcher.details.summary.export")}
          </button>
        </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16">
      ...
      <p className="mb-1 text-sm font-semibold text-muted-foreground/50">{t("launcher.details.summary.emptyTitle")}</p>
      <p className="mb-5 text-xs text-muted-foreground/40">{t("launcher.details.summary.emptyDescription")}</p>
      <button
        onClick={generation.generate}
      ...
      >
        ...
        {t("launcher.details.summary.generate")}
      </button>
    </div>
  );
}
